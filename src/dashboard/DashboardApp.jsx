    ...
    <>
      ...
      <main>
        <div className="page-header">
          ...
          <button 
            type="button" 
            className={`btn-add ${isRefreshing ? 'spinning' : ''}`}
            onClick={() => loadUrls(true)}
            disabled={isRefreshing}
            style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 16px', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border)', color: 'var(--text)' }}
          >
        ...
        <div className="url-list">
          {loading ? (
            <div className="empty-state" style={{ borderStyle: 'solid' }}>
              ...
              <p style={{ marginTop: '12px' }}>Loading node monitors...</p>
            </div>
          ) : urls.length === 0 ? (
            <div className="empty-state">
              ...
              <h3 className="empty-title">No URLs monitored yet</h3>
              <p className="empty-desc">Add your first URL above to start monitoring its uptime and performance latency.</p>
            </div>
          ) : (
            urls.map((url, idx) => {
                    ...
                    <div className="url-info">
                      ...
                      <div className="url-meta">
                        ...
                        <div className="url-meta-item">
                          ...
                          <span>{history.length} pings</span>
                        </div>
                      </div>
                    </div>
                    <div className={`status-badge ${status}`}>{status}</div>
                    
                    <svg className={`expand-icon ${isExpanded ? 'open' : ''}`} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ transition: 'transform 0.2s ease', transform: isExpanded ? 'rotate(180deg)' : 'rotate(0deg)' }}>
      ...
      <div className="toasts-container" id="toast-container">
        {toasts.map(toast => (
